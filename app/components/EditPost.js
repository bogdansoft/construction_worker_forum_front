    ...
    useEffect(() => {
        if (state.sendCount) {
            ...
            if (!state.title || state.title.length >= 50 || state.title.length < 3) {
                ...
                return
            } else if (!state.content || state.content.length >= 50 || state.content.length < 3) {
        ...
        if (state.topic.id !== selectedTopic.id) {
            return (
                <div className="ml-auto col-4"
                     style={{color: "FireBrick", font: "small-caps bold 14px/30px Georgia, serif"}}>
                ...
                <div className="content d-flex flex-column mt-4">
                    ...
                    <div className="d-flex flex-row">
                        <div className="ml-3 add-post-title">
                            Title: <input onChange={e => dispatch({type: "titleChange", value: e.target.value})}
                                          value={state.title} className="p-2 ml-3" type="text"/>
                        </div>
                    ...
                    <div className="mt-3 ml-auto mr-auto">
                        <textarea onChange={e => dispatch({type: "contentChange", value: e.target.value})}
                                  value={state.content} className="post-textarea p-2 ml-5" rows="10"
                                  cols="100"></textarea>
                    </div>
