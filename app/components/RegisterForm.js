  ...
  useEffect(() => {
    if (state.submitCount) {
      ...
      async function register() {
        try {
          const response = await Axios.post("http://localhost:8080/user", { username: state.username.value, password: state.password.value, email: state.email.value, firstName: state.firstName.value, lastName: state.lastName.value }, { cancelToken: ourRequest.token })
