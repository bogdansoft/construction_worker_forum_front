...
function ViewSinglePost() {
            ...
            async function fetchLikePostData() {
                try {
                    if (!state.isPostLikedByUser) {
                        ...
                        appDispatch({
                            type: "flashMessage",
                            value: "Post unliked successfully!",
                            messageType: "message-green"
                        })
                        setState(draft => {
    ...
    function showDeleteButton() {
        if (loggedIn && (appState.user.id == state.author.id || appState.user.isAdmin || appState.user.isSupport)) {
            return (
                <a>
          <span onClick={deletePopup} className="material-symbols-outlined link-black" data-tip="Delete"
                data-for="delete">
            delete
          </span>
                        ...
                        <div class="delete-absolute container col-5 ml-1 mt-5">
                            <div className="delete-pop col-5 p-2 liveValidateMessage-delete">
                                <DeleteModal delete={handleDelete} noDelete={deletePopup}
                                             relatedItemsLength={comments.length} relatedItemsType={"comment"}/>
                            </div>
                        </div>
                    </CSSTransition>
                    ...
                    <div>
                        <div className="container d-flex flex-row mt-3">
                            ...
                            <div className="ml-auto">
                                ...
                                <a onClick={!state.isPostOwnedByUser ? () => setState(draft => {
                                    draft.like++
                                }) : null}>
                                    <LikeButton isLiked={state.isPostLikedByUser}
                                                isOwner={state.isPostOwnedByUser}></LikeButton>
                                </a>
                                <div className="mobile-toggle-inverse">
            ...
            {loggedIn ? (<>
                    <div className="comments d-flex col-11 ml-auto mr-auto mt-5 align-items-center">
                        <form onSubmit={handleSubmit} className="d-flex ml-auto mr-auto align-items-center container">
                            ...
                            <CSSTransition in={state.commentToAdd.hasErrors} timeout={330}
                                           classNames="liveValidateMessage" unmountOnExit>
                                <div
                                    className="alert alert-danger small liveValidateMessage ml-3">{state.commentToAdd.message}</div>
                            </CSSTransition>
                            <div className="ml-auto mr-4 mt-4">
                                <button type="submit" className="material-symbols-outlined" data-tip="Send comment!"
                                        data-for="send">
                                    send
                                </button>
                    ...
                    <div
                        className="comments d-flex flex-column ml-auto mr-auto col-11 mt-5">{comments.length > 0 ? comments.map(comment =>
                            <SingleComment comment={comment} key={comment.id} reload={reload}/>) :
                        <div className="single-topic container d-flex mt-4">No comments yet!</div>}</div>
                </>
            ) : null}
        </div>
    )
